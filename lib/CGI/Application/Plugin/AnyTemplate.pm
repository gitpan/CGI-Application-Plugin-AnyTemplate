...
L<HTML::Template::Expr>, L<Template::Toolkit> and L<Petal>.
...
=head2 Component Dispatch

In addition to template abstraction, C<AnyTemplate> also provides a
I<component dispatch mechanism>.  For instance, you might include a
...
below under L<COMPONENT DISPATCH> for details.
...
C<AnyTemplate> driver (see below under L<Driver Configuration> for how
to set this).
...
Normally, component dispatching is handled by
L<CGI::Application::Plugin::AnyTemplate::Dispatcher>.  If you want to
...
L<CGI::Application::Plugin::AnyTemplate::Dispatcher>.  See the source
...
sub load {
    ...
    my $driver = $driver_class->_new(
         ...
         'webapp'           => $self->{'webapp'},
         'dispatcher_class' => $plugin_config->{'dispatcher_class'},
    );
...
template are run.  See L<COMPONENT DISPATCH>, below.
...
The syntax for dispatching to embedded components is specific to each
type of template driver.

=head2 Syntax

L<HTML::Template> syntax:

    <TMPL_VAR NAME="CGIAPP_dispatch('some_run_mode')">

L<HTML::Template::Expr> syntax:

    <TMPL_VAR EXPR="CGIAPP_dispatch('some_run_mode')">

L<Template::Toolkit> syntax:

    [% CGIAPP.dispatch("some_run_mode") %]

L<Petal> syntax:

    <span tal:replace="structure CGIAPP/dispatch 'some_run_mode'">
...
L<HTML::Template> syntax:
    ...
    <TMPL_VAR NAME="CGIAPP_dispatch('some_run_mode', param1, 'literal string2')">
...
I<see the docs to> L<CGI::Application::AnyTemplate::Driver::HTMLTemplate>
I<for limitations to the emulation>.

L<HTML::Template::Expr> syntax:

    <TMPL_VAR EXPR="CGIAPP_dispatch('some_run_mode', param1, 'literal string2')">

L<Template::Toolkit> syntax:

    [% CGIAPP.dispatch("some_run_mode", param1, 'literal string2' ) %]

L<Petal> syntax:

    <span tal:replace="structure CGIAPP/dispatch 'some_run_mode' param1 'literal string2' ">
        this text gets replaced by the output of some_run_mode
    </span>
