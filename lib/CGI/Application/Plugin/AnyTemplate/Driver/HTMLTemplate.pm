...
CGI::Application::Plugin::Driver::AnyTemplate::HTMLTemplate - HTML::Template driver to AnyTemplate
...
=head1 COMPONENT DISPATCH SYNTAX (HTML::Template)

=head2 Syntax

The L<HTML::Template> syntax for component dispatch is:

    <TMPL_VAR NAME="cgiapp_dispatch('some_run_mode', param1, param2, 'literal string3')">
...
The parameter list passed to the dispatch subroutine is parsed before
...
replacing any magic C<*dispatch*> tags with the content
generated by the appropriate runmodes.
...
sub render_template {
    ...
    my $tag_match = '^'
                  . quotemeta(
                       $driver_config->{'dispatch_tag_name'}
                    )
                  . '\s*'
                  . '\((.*?)\)?'   # optional params
                  . '\s*'
    ...
    # fill the CGIAPP_dispatch(foo,...) tags
    foreach my $tag ($template->query) {
        ...
        if ($tag =~ $tag_match) {
            ...
            $self->param($tag => ${ $dispatcher->dispatch_direct($run_mode, @prepped_params) });
        }
    }
