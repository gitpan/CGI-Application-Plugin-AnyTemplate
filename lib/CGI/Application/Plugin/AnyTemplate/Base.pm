...
sub _new {
    ...
    $self->{'dispatcher_class'} = delete $args{'dispatcher_class'}
...
template are run.  See C<COMPONENT DISPATCH>, below.
...
sub output {
    ...
    if ($self->{'webapp'} and UNIVERSAL::can($self->{'webapp'}, 'template_pre_process')) {
        $self->{'webapp'}->template_pre_process($self);
    }

    my $output = $self->render_template;

    if ($self->{'webapp'} and UNIVERSAL::can($self->{'webapp'}, 'template_post_process')) {
        my $output_param = $output;
        $output_param = \$output_param unless ref $output_param;
        $self->{'webapp'}->template_post_process($output_param);
    }
    return $output;
...
It also must manage dispatch to nested components.
